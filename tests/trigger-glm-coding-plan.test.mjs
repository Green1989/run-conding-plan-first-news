...
test('workflow config schedules the job for 04:30 Beijing time and wires the secret', async () => {
  const workflow = await fs.readFile(workflowPath, 'utf8');

  assert.match(workflow, /workflow_dispatch:/);
  assert.match(workflow, /cron:\s*'30 20 \* \* \*'/);
...
test('README documents the required secret, UTC conversion, retries, and manual trigger', async () => {
  ...
  assert.match(readme, /04:30/);
  assert.match(readme, /20:30 UTC/);
